...
tmn.array <- ncvar_get(ncin,dname)
...
tmn.array[tmn.array == fillvalue$value] <- NA

m <- 1
tmn.slice <- tmn.array[ , , m]

lonlat <- expand.grid(lon, lat)  ##important function
tmn.vec <- as.vector(tmn.slice)
length(tmn.vec)

tmn.vec.long <- as.vector(tmn.array)

tmn.mat <- matrix(tmn.vec.long, nrow = nlon * nlat, ncol = nt)
eval.tmn.mat <- tmn.mat[,1033:1392]
tmn.df.mean <- data.frame(cbind(lonlat,eval.tmn.mat))
min.tmn <- as.data.frame(apply(tmn.df.mean[3:362],1,min))
results.tmn <- cbind(lonlat, min.tmn )
results.tmn.omit.na <- na.omit(results.tmn)
colnames(results.tmn.omit.na) <- c("lon", "lat", "tmn")

china.tmn.omit.na <- subset(results.tmn.omit.na,lon>83 & lon <  130 & lat < 50 & lat > 20)
